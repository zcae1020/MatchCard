...
class channelManager {
    async createChannel(uid, maxRoom, maxTeam) {
        //firebase에서 channel 저장, id는 firebase에서 만들어준 id사용4
        return new Promise(async (resolve, reject) => {
            await GM.getGroupByUid(uid)
                .then((group) => {
    ...
    getChannelById(channelId) {
        //firebase에서 id로 channel 가져오기
        return new Promise((resolve, reject) => {
            channelRef.child(`/${channelId}`).on(
                "value",
                (snapshot) => {
    ...
    getChannelListInGroupByUid(uid) {
        // group이 관리하는 channelId들 가져오기
        return new Promise((resolve, reject) => {
            GM.getGroupByUid(uid).then((group) => {
                let groupId = group["groupId"];
                groupRef.child(`/${groupId}/channels`).on(
                    "value",
                    async (snapshot) => {
                        let ret = [];
    ...
    deleteChannelById(channelId) {
        //firebase에서 id로 channel 지우기
        return new Promise((resolve, reject) => {
            GM.getGroupByChannelId(channelId)
                .then((group) => {
                    let groupId = group["groupId"];
